...
$(document).ready(function() {
    ...
    async function handleCheckHouseForm(event) {
        ...
        const housePassword = $('#housePassword').val();
        if (houseName === '' || housePassword === '') {
            $('#result-message').text(translations[currentLang].enterHouseName || 'الرجاء إدخال اسم البيت وكلمة المرور').addClass('error');
            return;
        }

        // جلب بيانات البيت من checkHouse
        const response = await window.checkHouse(houseName);
        if (response.status === 'success') {
            // تحقق من كلمة المرور
            const apiKey = window.SUPABASE_ANON_KEY;
            const url = 'https://akvyhsmobalbqfcjupdq.supabase.co/rest/v1/homes?id=eq.' + encodeURIComponent(response.houseId);
            try {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    const pwData = encoder.encode(housePassword);
                    ...
                    if (inputHash === passwordHash) {
                        ...
                        $('#result-message').text('كلمة المرور غير صحيحة').removeClass('success').addClass('error');
                    }
                } else {
                    $('#result-message').text('حدث خطأ في جلب بيانات البيت').addClass('error');
                }
            } catch (e) {
                $('#result-message').text('حدث خطأ في الاتصال').addClass('error');
            }
        } else {
